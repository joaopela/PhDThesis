...
Similarly to the \gls{ECAL} the energy resolution \gls{HCAL} was tested using a test beam of single charged pions\cite{ARTICLE:CMSECALTestBeam}, and it was obtained.
...
The \gls{HLT} has as it input the events coming from the \gls{L1T} and needs to preform further event reduction of 
$\mathcal{O}(10^{5})$ to a final output rate of $\mathcal{O}(10^{2-3})\,\hertz$ . This system is composed of standard computing hardware in the form of computing farm with several thousands of \gls{CPU}. This system, using the additional latency created by the \gls{L1T} event selection, is able to make use of the complete detector information including the tracker data. More sophisticated and precise algorithms are therefore possible which can be tailored to any desired physical final state. 

Both event selection algorithms at the \gls{L1T} and \gls{HLT} are constantly updated during data taking. Many times it necessary to update the selection thresholds in order to control the rate with the increase of \gls{LHC} luminosity, but such changes can also be due to the development of novel methods or strategies to identify particles more efficiently or the interest for new final final states. A set of algorithms used for data taking is normally referred as the \textit{trigger menu}. 

After events passing both levels of trigger they are recorded in permanent storage. During 2012-13 operation two output streams were saved. The \textit{prompt data stream}, with a rate of approximately $300\,\hertz$, was composed of high priority trigger paths would be immediately reconstructed. And the \textit{parked data stream}, with a rate up to $1\,\kilo\hertz$, was stored without reconstruction. This data would wait until computing resourced would be free to go through reconstruction\cite{ARTICLE:CMSDataParking}. This process was finished a few months after run I was finished.

Even with such measure to reduce the data to be stored, each \gls{LHC} experiment record several petabytes of data every year in addition to similarly sized amounts of simulated events.
...
To ensure physics performance during 2015 only a partial upgrade is planned for the 2015 run, which is known at the \textit{Stage-1} upgrade system. The main feature of this upgrade is the replacement of the existing \gls{GCT}. Two key enhancements will be possible from the upgrade: 
...
The intermediate system will have significantly better performance than the now legacy system. The full 2016 calorimeter trigger system will additionally provide finer granularitym which will lead to increased position and energy resolution. 
