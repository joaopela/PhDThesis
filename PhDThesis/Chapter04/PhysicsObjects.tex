...
Different algorithms are used in the barrel and endcaps regions. In the barrel the region we explore the simple $\eta-\phi$ geometry with the \textit{hybrid clustering algorithm}. The procedure started by identifying \textit{seed crystals} with $E_\perp>1\,\GeV$. We form a domino around this seed in the $\eta$ direction of $3 \times 1$ or $5 \times 1$ crystals centred at the seed. Additional dominoes are added in both $\phi$ direction in an attempt to collect the bremsstrahlung emissions up to $\Delta\phi \approx 0.3$. Any domino with energy below $100\,\MeV$ is disregarded. The resulting additional sub-clusters must have its own seed with $E_\perp>350\,\MeV$ and they are combined to for the final \textit{supercluster}


In the encaps the \textit{island algorithm} is used, 

\cite{ARTICLE:CMSElectronReconstruction8TeV}
