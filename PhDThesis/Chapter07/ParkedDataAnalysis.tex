...
% \label{sec:syst}
% The dominant source of uncertainty is the statistical
% uncertainty relating to the finite number of events in both data and simulation in each of the control regions used for the normalisation of the different backgrounds. In order to estimate the effect of the jet energy scale,
...
% uncertainties~\cite{Chatrchyan:2011ds}, the \MET is recalculated and the full background and signal estimations are
% repeated. Similarly, the lepton efficiency and pileup scale factors
% applied to the MC are varied by their uncertainties \cite{Chatrchyan:2012xi,CMS:2013hoa} and the analysis is repeated.
% 
% The integrated luminosity uncertainty of 2.6\% is applied only to the signal and to the 
% backgrounds estimated from MC. The data-based normalisation
% for the main backgrounds also ensures that the trigger efficiency is properly
% taken into account and its impact on the signal and minor backgrounds was found to be negligible.
% 
% The uncertainties on the diboson cross sections are taken from the CMS published cross section
% measurements~\cite{Chatrchyan:2013oev}. Theoretical uncertainties on the signal cross sections due to PDF and QCD scales are taken from the LHC
% Higgs Cross Section Working Groups Yellow Report 3~\cite{Dittmaier:2011ti,Dittmaier:2012vm}.
% 
% The extrapolation of the $Z\rightarrow\nu\nu$ background from the QCD production of $Z/\gamma^{*}\rightarrow\mu\mu$ has been tested by comparing the \MADGRAPH and aMC@NLO\_MG5 \cite{Alwall:2014hca} predictions. Both were found to be compatible within the statistical uncertainty of the study and, therefore, no additional theoretical uncertainty has been added.
% 
% The overall size of these uncertainties, as a percentage of the total signal and background estimates, can be found in Table~\ref{tab:syst-qqH}.
% \section{Results}
% \label{sec:results}
