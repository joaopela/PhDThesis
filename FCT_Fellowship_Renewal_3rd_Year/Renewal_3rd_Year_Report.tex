...
%Title of paper
\title{Activities Report for the 2nd year of the FCT PhD Fellowship SFRH/BD/77979/2011}
...
separated into mutually exclusive categories of different expected signal-to-background ratios. This selection is
based on the properties of the reconstructed photons and on the presence of two jets satisfying criteria aimed at 
selecting events in which a Higgs boson is produced through the VBF process. The analysis uses multivariate techniques 
...
This choice of function introduces a systematic error since we do not know the underlying function in the processes 
produce and smear our data. Thus implies the need to study how much this choice changes our background estimation.
...
\begin{enumerate}
 ...
 \item We can now extract the global mean, which is an approximation of the systematic error associated with the 
 choice of function and global RMS which is an approximation of the statistical error associated with the initial data.
\end{enumerate}

\begin{figure}[ht]
\centering
\includegraphics[width=0.45\textwidth]{img/sig122to128_Truth-fPow2_vs_fExp2.png}
\caption{Plot of the diference of estimation of background contribution on the signal area, made by fitting toys from a generating
...
The following reference frames were analysed: Collins-Soper, Helicity, Perpendicular Helicity, Gottfried-Jackson and 
...
At the beginning of 2012 the possibility of recording data without prompt reconstruction it was introduced, thus 
allowing to record up to 1500 $Hz$ of data where only 300 $Hz$ are promptly reconstructed. This is now know as
\textit{parked data}; this data was reconstructed after the beginning of \textit{Long Shutdown 1} already in 2013.
...
For such a trigger to work it would have to be based only on the forward dijet which is the defining characteristic
...
on the L1 hardware; according to trigger experts it is in principle possible to implement with the current hardware.
...
for the experiment.
It is possible to do service work in several ways, from providing direct contribution to the operation of
...
person for the L1 trigger, who is responsible for supporting the online operation of this subsystem. In the case of
...
\begin{itemize}
  ...
  \item: Summary plots: several summary plots are automatically made to allow the online shifter of certification 
...
\bibliography{Renewal_3rd_Year_Bibliography}
